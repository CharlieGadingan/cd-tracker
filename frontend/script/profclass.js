...
function setupEventListeners() {
    ...
    if (submissionFilter) {
        submissionFilter.addEventListener('change', (event) => {
            state.submissionFilter = String(event.target.value || 'ALL').toUpperCase();
    ...
    if (submittedDetailBody) {
        submittedDetailBody.addEventListener('click', async (event) => {
            ...
            const action = actionButton.getAttribute('data-action');
            if (action !== 'copy-submission-url' && action !== 'copy-clone-command') return;
            ...
            if (action === 'copy-clone-command') {
                await copyTextWithFeedback(`git clone ${row.repositoryUrl}`, 'Clone command copied.');
            }
        });
    }

    const submittedDetailBack = document.getElementById('closeSubmittedDetailBtn');
...
async function loadUserProfile() {
    try {
        ...
        const profile = result?.data && typeof result.data === 'object' && !result.firstName
            ? result.data
            : result;
...
async function loadStudents() {
    try {
        const result = await apiRequest(`/classrooms/${encodeURIComponent(state.classroomId)}/students`, {
            method: 'GET'
        });
...
async function loadActivities() {
    try {
        const result = await apiRequest(`/classrooms/${encodeURIComponent(state.classroomId)}/activities/owner`, {
            method: 'GET'
        });
...
async function loadSubmittedActivities() {
    try {
        const result = await apiRequest(`/classrooms/${encodeURIComponent(state.classroomId)}/activities/submitted`, {
            method: 'GET'
        });
        ...
        showNotification(error?.message || 'Failed to load submitted activities.', 'error');
    }
}
...
function mapSubmittedByActivity(payload) {
    ...
    Object.entries(payload).forEach(([userKey, userEntry]) => {
        ...
        const studentActivities = Array.isArray(userEntry.studentActivities) ? userEntry.studentActivities : [];
...
function normalizeSubmittedEntry(entry, fallback) {
    ...
    const parsedScore = scoreRaw == null || scoreRaw === '' ? null : Number(scoreRaw);

    const maxScoreRaw = entry?.maxScore;
    const parsedMaxScore = maxScoreRaw == null || maxScoreRaw === '' ? null : Number(maxScoreRaw);
...
function renderActivities() {
    ...
    container.innerHTML = state.activities.map((activity) => {
        ...
        const maxScore = activity?.maxScore != null ? Number(activity.maxScore) : null;
...
function buildSubmissionRows(activityId) {
    ...
    Object.values(perActivity).forEach((entry) => {
        ...
        const fullName = student?.displayName || asString(entry?.displayName) || 'Student';
        rows.push({
            activityId,
            userId,
...
function deriveSubmissionStatus(entry) {
    ...
    if (rawStatus === 'PENDING' || rawStatus === 'SUBMITTED' || rawStatus === 'GRADED') {
        return rawStatus;
    }
...
function renderSubmissionsModal() {
    const activity = state.activities.find((item) => getActivityId(item) === state.currentSubmissionsActivityId);
    const titleEl = document.getElementById('submissionsModalTitle');
    const subtitleEl = document.getElementById('submissionsSubtitle');

    state.currentSubmissionRows = buildSubmissionRows(state.currentSubmissionsActivityId);
...
function renderSubmissionRows() {
    ...
    const rows = state.currentSubmissionRows.filter((row) => filter === 'ALL' || row.status === filter);
    ...
    container.innerHTML = rows.map((row) => {
        ...
        const gradedMeta = row.status === 'GRADED'
            ? `
                    <div>
        ...
        if (row.status === 'PENDING') {
            actionArea = '<span class="inline-note">Waiting final activity submission</span>';
...
function openSubmissionDetailModal(activityId, studentUserId) {
    const row = state.currentSubmissionRows.find((item) => item.activityId === activityId && item.userId === studentUserId);
    if (!row) {
    ...
    const gradingMessage = status === 'GRADED'
...
function openGradeModal(activityId, studentUserId) {
    ...
    const activity = state.activities.find((item) => getActivityId(item) === activityId);
    const effectiveMaxScore = row.maxScore != null
        ? Number(row.maxScore)
    ...
    setInputValue('gradeMaxScore', effectiveMaxScore != null ? String(effectiveMaxScore) : '');
    ...
    if (scoreHint) {
        scoreHint.textContent = effectiveMaxScore != null && Number.isFinite(effectiveMaxScore)
...
async function handleSubmitGrade() {
    ...
    const targetRow = state.currentSubmissionRows.find((item) => item.activityId === activityId && item.userId === studentUserId);
    ...
    if (scoreRaw) {
        ...
        if (maxScoreRaw) {
            ...
            if (Number.isFinite(maxScore) && score > maxScore) {
                showNotification(`Score cannot exceed max score (${maxScore}).`, 'error');
                return;
            }
        }
...
async function handleCreateActivity() {
    ...
    try {
        await apiRequest(`/classrooms/${encodeURIComponent(state.classroomId)}/activities`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function openEditActivityModal(activityId) {
    ...
    setInputValue('editMaxScore', activity.maxScore != null ? String(activity.maxScore) : '');
...
async function handleEditActivity() {
    ...
    try {
        await apiRequest(`/classrooms/${encodeURIComponent(state.classroomId)}/activities/${encodeURIComponent(activityId)}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteActivity(activityId) {
    const confirmMessage = 'Are you sure you want to delete this activity? This cannot be undone.';
    let approved = false;
...
function getDueInfo(dueDate) {
    ...
    return { label: `${dueText} (${diffDays} day${diffDays === 1 ? '' : 's'} left)` };
}
